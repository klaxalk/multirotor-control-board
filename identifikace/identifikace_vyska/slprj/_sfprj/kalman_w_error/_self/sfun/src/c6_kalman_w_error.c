...
static void initialize_c6_kalman_w_error(SFc6_kalman_w_errorInstanceStruct
  *chartInstance);
static void initialize_params_c6_kalman_w_error
  (SFc6_kalman_w_errorInstanceStruct *chartInstance);
static void enable_c6_kalman_w_error(SFc6_kalman_w_errorInstanceStruct
  *chartInstance);
static void disable_c6_kalman_w_error(SFc6_kalman_w_errorInstanceStruct
  *chartInstance);
static void c6_update_debugger_state_c6_kalman_w_error
  (SFc6_kalman_w_errorInstanceStruct *chartInstance);
static const mxArray *get_sim_state_c6_kalman_w_error
  (SFc6_kalman_w_errorInstanceStruct *chartInstance);
static void set_sim_state_c6_kalman_w_error(SFc6_kalman_w_errorInstanceStruct
  *chartInstance, const mxArray *c6_st);
static void finalize_c6_kalman_w_error(SFc6_kalman_w_errorInstanceStruct
  *chartInstance);
static void sf_c6_kalman_w_error(SFc6_kalman_w_errorInstanceStruct
  *chartInstance);
static void c6_chartstep_c6_kalman_w_error(SFc6_kalman_w_errorInstanceStruct
  *chartInstance);
static void initSimStructsc6_kalman_w_error(SFc6_kalman_w_errorInstanceStruct
  *chartInstance);
static void init_script_number_translation(uint32_T c6_machineNumber, uint32_T
  c6_chartNumber);
static const mxArray *c6_sf_marshallOut(void *chartInstanceVoid, void *c6_inData);
static real_T c6_emlrt_marshallIn(SFc6_kalman_w_errorInstanceStruct
  *chartInstance, const mxArray *c6_cyclesBlind, const char_T *c6_identifier);
static real_T c6_b_emlrt_marshallIn(SFc6_kalman_w_errorInstanceStruct
...
static void c6_eml_scalar_eg(SFc6_kalman_w_errorInstanceStruct *chartInstance);
static const mxArray *c6_b_sf_marshallOut(void *chartInstanceVoid, void
  *c6_inData);
static int32_T c6_c_emlrt_marshallIn(SFc6_kalman_w_errorInstanceStruct
  *chartInstance, const mxArray *c6_u, const emlrtMsgIdentifier *c6_parentId);
static void c6_b_sf_marshallIn(void *chartInstanceVoid, const mxArray
  *c6_mxArrayInData, const char_T *c6_varName, void *c6_outData);
static uint8_T c6_d_emlrt_marshallIn(SFc6_kalman_w_errorInstanceStruct
  *chartInstance, const mxArray *c6_b_is_active_c6_kalman_w_error, const char_T *
  c6_identifier);
static uint8_T c6_e_emlrt_marshallIn(SFc6_kalman_w_errorInstanceStruct
  *chartInstance, const mxArray *c6_u, const emlrtMsgIdentifier *c6_parentId);
static void init_dsm_address_info(SFc6_kalman_w_errorInstanceStruct
  *chartInstance);
  ...
  *chartInstance)
{
  ...
  chartInstance->c6_is_active_c6_kalman_w_error = 0U;
}
  ...
  (SFc6_kalman_w_errorInstanceStruct *chartInstance)
{
  ...
  c6_d_hoistedGlobal = chartInstance->c6_is_active_c6_kalman_w_error;
  ...
  *chartInstance, const mxArray *c6_st)
{
  ...
  *c6_good = c6_emlrt_marshallIn(chartInstance, sf_mex_dup(sf_mex_getcell(c6_u,
    2)), "good");
  chartInstance->c6_is_active_c6_kalman_w_error = c6_d_emlrt_marshallIn
    (chartInstance, sf_mex_dup(sf_mex_getcell(c6_u, 3)),
     "is_active_c6_kalman_w_error");
  sf_mex_destroy(&c6_u);
  c6_update_debugger_state_c6_kalman_w_error(chartInstance);
  ...
  *chartInstance)
{
  ...
  chartInstance->c6_sfEvent = CALL_EVENT;
  c6_chartstep_c6_kalman_w_error(chartInstance);
...
static const mxArray *c6_sf_marshallOut(void *chartInstanceVoid, void *c6_inData)
{
  ...
  const mxArray *c6_y = NULL;
  SFc6_kalman_w_errorInstanceStruct *chartInstance;
  chartInstance = (SFc6_kalman_w_errorInstanceStruct *)chartInstanceVoid;
...
const mxArray *sf_c6_kalman_w_error_get_eml_resolved_functions_info(void)
{
  ...
  c6_identifier)
{
  ...
  c6_y = c6_e_emlrt_marshallIn(chartInstance, sf_mex_dup
    (c6_b_is_active_c6_kalman_w_error), &c6_thisId);
  sf_mex_destroy(&c6_b_is_active_c6_kalman_w_error);
  return c6_y;
}
...
void sf_c6_kalman_w_error_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c6_kalman_w_error_get_autoinheritance_info(void)
{
...
mxArray *sf_c6_kalman_w_error_third_party_uses_info(void)
{
  mxArray * mxcell3p = mxCreateCellMatrix(1,0);
  return(mxcell3p);
}

mxArray *sf_c6_kalman_w_error_updateBuildInfo_args_info(void)
{
  mxArray *mxBIArgs = mxCreateCellMatrix(1,0);
  return mxBIArgs;
}

static const mxArray *sf_get_sim_state_info_c6_kalman_w_error(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  const char *infoEncStr[] = {
    "100 S1x4'type','srcId','name','auxInfo'{{M[1],M[7],T\"confidence\",},{M[1],M[10],T\"cyclesBlind\",},{M[1],M[5],T\"good\",},{M[8],M[0],T\"is_active_c6_kalman_w_error\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 4, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c6_kalman_w_error_get_check_sum(&mxChecksum);
  ...
  fullDebuggerInitialization)
{
  if (!sim_mode_is_rtw_gen(S)) {
    SFc6_kalman_w_errorInstanceStruct *chartInstance;
    chartInstance = (SFc6_kalman_w_errorInstanceStruct *) ((ChartInfoStruct *)
...
static void sf_opaque_initialize_c6_kalman_w_error(void *chartInstanceVar)
{
  chart_debug_initialization(((SFc6_kalman_w_errorInstanceStruct*)
    chartInstanceVar)->S,0);
  initialize_params_c6_kalman_w_error((SFc6_kalman_w_errorInstanceStruct*)
    chartInstanceVar);
  initialize_c6_kalman_w_error((SFc6_kalman_w_errorInstanceStruct*)
    chartInstanceVar);
}

static void sf_opaque_enable_c6_kalman_w_error(void *chartInstanceVar)
{
  enable_c6_kalman_w_error((SFc6_kalman_w_errorInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_disable_c6_kalman_w_error(void *chartInstanceVar)
{
  disable_c6_kalman_w_error((SFc6_kalman_w_errorInstanceStruct*)
    chartInstanceVar);
}

static void sf_opaque_gateway_c6_kalman_w_error(void *chartInstanceVar)
{
  sf_c6_kalman_w_error((SFc6_kalman_w_errorInstanceStruct*) chartInstanceVar);
}

extern const mxArray* sf_internal_get_sim_state_c6_kalman_w_error(SimStruct* S)
{
  ...
  prhs[2] = (mxArray*) get_sim_state_c6_kalman_w_error
    ((SFc6_kalman_w_errorInstanceStruct*)chartInfo->chartInstance);/* raw sim ctx */
  prhs[3] = (mxArray*) sf_get_sim_state_info_c6_kalman_w_error();/* state var info */
...
extern void sf_internal_set_sim_state_c6_kalman_w_error(SimStruct* S, const
  mxArray *st)
{
  ChartInfoStruct *chartInfo = (ChartInfoStruct*) ssGetUserData(S);
  mxArray *plhs[1] = { NULL };
  ...
  set_sim_state_c6_kalman_w_error((SFc6_kalman_w_errorInstanceStruct*)
    chartInfo->chartInstance, mxDuplicateArray(plhs[0]));
  mxDestroyArray(plhs[0]);
}

static const mxArray* sf_opaque_get_sim_state_c6_kalman_w_error(SimStruct* S)
{
  return sf_internal_get_sim_state_c6_kalman_w_error(S);
}

static void sf_opaque_set_sim_state_c6_kalman_w_error(SimStruct* S, const
  mxArray *st)
{
  sf_internal_set_sim_state_c6_kalman_w_error(S, st);
}

static void sf_opaque_terminate_c6_kalman_w_error(void *chartInstanceVar)
{
  if (chartInstanceVar!=NULL) {
    SimStruct *S = ((SFc6_kalman_w_errorInstanceStruct*) chartInstanceVar)->S;
    if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
      sf_clear_rtw_identifier(S);
      unload_kalman_w_error_optimization_info();
    }

    finalize_c6_kalman_w_error((SFc6_kalman_w_errorInstanceStruct*)
      chartInstanceVar);
...
{
  initSimStructsc6_kalman_w_error((SFc6_kalman_w_errorInstanceStruct*)
    chartInstanceVar);
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c6_kalman_w_error(SimStruct *S)
{
  int i;
...
static void mdlSetWorkWidths_c6_kalman_w_error(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
...
static void mdlRTW_c6_kalman_w_error(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
...
static void mdlStart_c6_kalman_w_error(SimStruct *S)
{
  SFc6_kalman_w_errorInstanceStruct *chartInstance;
  chartInstance = (SFc6_kalman_w_errorInstanceStruct *)utMalloc(sizeof
    (SFc6_kalman_w_errorInstanceStruct));
  memset(chartInstance, 0, sizeof(SFc6_kalman_w_errorInstanceStruct));
  ...
  chartInstance->chartInfo.sFunctionGateway =
    sf_opaque_gateway_c6_kalman_w_error;
  chartInstance->chartInfo.initializeChart =
    sf_opaque_initialize_c6_kalman_w_error;
  chartInstance->chartInfo.terminateChart =
    sf_opaque_terminate_c6_kalman_w_error;
  chartInstance->chartInfo.enableChart = sf_opaque_enable_c6_kalman_w_error;
  chartInstance->chartInfo.disableChart = sf_opaque_disable_c6_kalman_w_error;
  chartInstance->chartInfo.getSimState =
    sf_opaque_get_sim_state_c6_kalman_w_error;
  chartInstance->chartInfo.setSimState =
    sf_opaque_set_sim_state_c6_kalman_w_error;
  chartInstance->chartInfo.getSimStateInfo =
    sf_get_sim_state_info_c6_kalman_w_error;
  chartInstance->chartInfo.zeroCrossings = NULL;
  chartInstance->chartInfo.outputs = NULL;
  chartInstance->chartInfo.derivatives = NULL;
  chartInstance->chartInfo.mdlRTW = mdlRTW_c6_kalman_w_error;
  chartInstance->chartInfo.mdlStart = mdlStart_c6_kalman_w_error;
  chartInstance->chartInfo.mdlSetWorkWidths = mdlSetWorkWidths_c6_kalman_w_error;
...
void c6_kalman_w_error_method_dispatcher(SimStruct *S, int_T method, void *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c6_kalman_w_error(S);
    break;

   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c6_kalman_w_error(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c6_kalman_w_error(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c6_kalman_w_error_method_dispatcher.\n"
