...
static void initialize_c4_kalman_w_error(SFc4_kalman_w_errorInstanceStruct
  *chartInstance);
static void initialize_params_c4_kalman_w_error
  (SFc4_kalman_w_errorInstanceStruct *chartInstance);
static void enable_c4_kalman_w_error(SFc4_kalman_w_errorInstanceStruct
  *chartInstance);
static void disable_c4_kalman_w_error(SFc4_kalman_w_errorInstanceStruct
  *chartInstance);
static void c4_update_debugger_state_c4_kalman_w_error
  (SFc4_kalman_w_errorInstanceStruct *chartInstance);
static const mxArray *get_sim_state_c4_kalman_w_error
  (SFc4_kalman_w_errorInstanceStruct *chartInstance);
static void set_sim_state_c4_kalman_w_error(SFc4_kalman_w_errorInstanceStruct
  *chartInstance, const mxArray *c4_st);
static void finalize_c4_kalman_w_error(SFc4_kalman_w_errorInstanceStruct
  *chartInstance);
static void sf_c4_kalman_w_error(SFc4_kalman_w_errorInstanceStruct
  *chartInstance);
static void initSimStructsc4_kalman_w_error(SFc4_kalman_w_errorInstanceStruct
  *chartInstance);
static void init_script_number_translation(uint32_T c4_machineNumber, uint32_T
  c4_chartNumber);
static const mxArray *c4_sf_marshallOut(void *chartInstanceVoid, void *c4_inData);
static real_T c4_emlrt_marshallIn(SFc4_kalman_w_errorInstanceStruct
  *chartInstance, const mxArray *c4_cyclesBlind, const char_T *c4_identifier);
static real_T c4_b_emlrt_marshallIn(SFc4_kalman_w_errorInstanceStruct
...
static void c4_eml_scalar_eg(SFc4_kalman_w_errorInstanceStruct *chartInstance);
static const mxArray *c4_b_sf_marshallOut(void *chartInstanceVoid, void
  *c4_inData);
static int32_T c4_c_emlrt_marshallIn(SFc4_kalman_w_errorInstanceStruct
  *chartInstance, const mxArray *c4_u, const emlrtMsgIdentifier *c4_parentId);
static void c4_b_sf_marshallIn(void *chartInstanceVoid, const mxArray
  *c4_mxArrayInData, const char_T *c4_varName, void *c4_outData);
static uint8_T c4_d_emlrt_marshallIn(SFc4_kalman_w_errorInstanceStruct
  *chartInstance, const mxArray *c4_b_is_active_c4_kalman_w_error, const char_T *
  c4_identifier);
static uint8_T c4_e_emlrt_marshallIn(SFc4_kalman_w_errorInstanceStruct
  *chartInstance, const mxArray *c4_u, const emlrtMsgIdentifier *c4_parentId);
static void init_dsm_address_info(SFc4_kalman_w_errorInstanceStruct
  *chartInstance);
  ...
  *chartInstance)
{
  ...
  chartInstance->c4_is_active_c4_kalman_w_error = 0U;
}
  ...
  (SFc4_kalman_w_errorInstanceStruct *chartInstance)
{
  ...
  c4_d_hoistedGlobal = chartInstance->c4_is_active_c4_kalman_w_error;
  ...
  *chartInstance, const mxArray *c4_st)
{
  ...
  *c4_good = c4_emlrt_marshallIn(chartInstance, sf_mex_dup(sf_mex_getcell(c4_u,
    2)), "good");
  chartInstance->c4_is_active_c4_kalman_w_error = c4_d_emlrt_marshallIn
    (chartInstance, sf_mex_dup(sf_mex_getcell(c4_u, 3)),
     "is_active_c4_kalman_w_error");
  sf_mex_destroy(&c4_u);
  c4_update_debugger_state_c4_kalman_w_error(chartInstance);
...
static const mxArray *c4_sf_marshallOut(void *chartInstanceVoid, void *c4_inData)
{
  ...
  const mxArray *c4_y = NULL;
  SFc4_kalman_w_errorInstanceStruct *chartInstance;
  chartInstance = (SFc4_kalman_w_errorInstanceStruct *)chartInstanceVoid;
...
const mxArray *sf_c4_kalman_w_error_get_eml_resolved_functions_info(void)
{
  ...
  c4_identifier)
{
  ...
  c4_y = c4_e_emlrt_marshallIn(chartInstance, sf_mex_dup
    (c4_b_is_active_c4_kalman_w_error), &c4_thisId);
  sf_mex_destroy(&c4_b_is_active_c4_kalman_w_error);
  return c4_y;
}
...
void sf_c4_kalman_w_error_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c4_kalman_w_error_get_autoinheritance_info(void)
{
...
mxArray *sf_c4_kalman_w_error_third_party_uses_info(void)
{
  mxArray * mxcell3p = mxCreateCellMatrix(1,0);
  return(mxcell3p);
}

mxArray *sf_c4_kalman_w_error_updateBuildInfo_args_info(void)
{
  mxArray *mxBIArgs = mxCreateCellMatrix(1,0);
  return mxBIArgs;
}

static const mxArray *sf_get_sim_state_info_c4_kalman_w_error(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  const char *infoEncStr[] = {
    "100 S1x4'type','srcId','name','auxInfo'{{M[1],M[7],T\"confidence\",},{M[1],M[10],T\"cyclesBlind\",},{M[1],M[5],T\"good\",},{M[8],M[0],T\"is_active_c4_kalman_w_error\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 4, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c4_kalman_w_error_get_check_sum(&mxChecksum);
  ...
  fullDebuggerInitialization)
{
  if (!sim_mode_is_rtw_gen(S)) {
    SFc4_kalman_w_errorInstanceStruct *chartInstance;
    chartInstance = (SFc4_kalman_w_errorInstanceStruct *) ((ChartInfoStruct *)
...
static void sf_opaque_initialize_c4_kalman_w_error(void *chartInstanceVar)
{
  chart_debug_initialization(((SFc4_kalman_w_errorInstanceStruct*)
    chartInstanceVar)->S,0);
  initialize_params_c4_kalman_w_error((SFc4_kalman_w_errorInstanceStruct*)
    chartInstanceVar);
  initialize_c4_kalman_w_error((SFc4_kalman_w_errorInstanceStruct*)
    chartInstanceVar);
}

static void sf_opaque_enable_c4_kalman_w_error(void *chartInstanceVar)
{
  enable_c4_kalman_w_error((SFc4_kalman_w_errorInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_disable_c4_kalman_w_error(void *chartInstanceVar)
{
  disable_c4_kalman_w_error((SFc4_kalman_w_errorInstanceStruct*)
    chartInstanceVar);
}

static void sf_opaque_gateway_c4_kalman_w_error(void *chartInstanceVar)
{
  sf_c4_kalman_w_error((SFc4_kalman_w_errorInstanceStruct*) chartInstanceVar);
}

extern const mxArray* sf_internal_get_sim_state_c4_kalman_w_error(SimStruct* S)
{
  ...
  prhs[2] = (mxArray*) get_sim_state_c4_kalman_w_error
    ((SFc4_kalman_w_errorInstanceStruct*)chartInfo->chartInstance);/* raw sim ctx */
  prhs[3] = (mxArray*) sf_get_sim_state_info_c4_kalman_w_error();/* state var info */
...
extern void sf_internal_set_sim_state_c4_kalman_w_error(SimStruct* S, const
  mxArray *st)
{
  ChartInfoStruct *chartInfo = (ChartInfoStruct*) ssGetUserData(S);
  mxArray *plhs[1] = { NULL };
  ...
  set_sim_state_c4_kalman_w_error((SFc4_kalman_w_errorInstanceStruct*)
    chartInfo->chartInstance, mxDuplicateArray(plhs[0]));
  mxDestroyArray(plhs[0]);
}

static const mxArray* sf_opaque_get_sim_state_c4_kalman_w_error(SimStruct* S)
{
  return sf_internal_get_sim_state_c4_kalman_w_error(S);
}

static void sf_opaque_set_sim_state_c4_kalman_w_error(SimStruct* S, const
  mxArray *st)
{
  sf_internal_set_sim_state_c4_kalman_w_error(S, st);
}

static void sf_opaque_terminate_c4_kalman_w_error(void *chartInstanceVar)
{
  if (chartInstanceVar!=NULL) {
    SimStruct *S = ((SFc4_kalman_w_errorInstanceStruct*) chartInstanceVar)->S;
    if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
      sf_clear_rtw_identifier(S);
      unload_kalman_w_error_optimization_info();
    }

    finalize_c4_kalman_w_error((SFc4_kalman_w_errorInstanceStruct*)
      chartInstanceVar);
...
{
  initSimStructsc4_kalman_w_error((SFc4_kalman_w_errorInstanceStruct*)
    chartInstanceVar);
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c4_kalman_w_error(SimStruct *S)
{
  int i;
...
static void mdlSetWorkWidths_c4_kalman_w_error(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
...
static void mdlRTW_c4_kalman_w_error(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
...
static void mdlStart_c4_kalman_w_error(SimStruct *S)
{
  SFc4_kalman_w_errorInstanceStruct *chartInstance;
  chartInstance = (SFc4_kalman_w_errorInstanceStruct *)utMalloc(sizeof
    (SFc4_kalman_w_errorInstanceStruct));
  memset(chartInstance, 0, sizeof(SFc4_kalman_w_errorInstanceStruct));
  ...
  chartInstance->chartInfo.sFunctionGateway =
    sf_opaque_gateway_c4_kalman_w_error;
  chartInstance->chartInfo.initializeChart =
    sf_opaque_initialize_c4_kalman_w_error;
  chartInstance->chartInfo.terminateChart =
    sf_opaque_terminate_c4_kalman_w_error;
  chartInstance->chartInfo.enableChart = sf_opaque_enable_c4_kalman_w_error;
  chartInstance->chartInfo.disableChart = sf_opaque_disable_c4_kalman_w_error;
  chartInstance->chartInfo.getSimState =
    sf_opaque_get_sim_state_c4_kalman_w_error;
  chartInstance->chartInfo.setSimState =
    sf_opaque_set_sim_state_c4_kalman_w_error;
  chartInstance->chartInfo.getSimStateInfo =
    sf_get_sim_state_info_c4_kalman_w_error;
  chartInstance->chartInfo.zeroCrossings = NULL;
  chartInstance->chartInfo.outputs = NULL;
  chartInstance->chartInfo.derivatives = NULL;
  chartInstance->chartInfo.mdlRTW = mdlRTW_c4_kalman_w_error;
  chartInstance->chartInfo.mdlStart = mdlStart_c4_kalman_w_error;
  chartInstance->chartInfo.mdlSetWorkWidths = mdlSetWorkWidths_c4_kalman_w_error;
...
void c4_kalman_w_error_method_dispatcher(SimStruct *S, int_T method, void *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c4_kalman_w_error(S);
    break;

   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c4_kalman_w_error(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c4_kalman_w_error(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c4_kalman_w_error_method_dispatcher.\n"
