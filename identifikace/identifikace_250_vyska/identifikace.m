...
position_pol = polyfit(time_plot', position, 50);
...
for i=-20:0.5:20
    ...
    action_fit = data(iden_window, input) - inputOffset + i;
    ...
    crit = sum((position_fit - position_est).^2);
...
action_pol = polyfit(time_plot', action, 70);
% action_fit = polyval(action_pol, time_identifikace);
action_fit = data(iden_window, input) - inputOffset + bestI;
...
plot(time_identifikace, position_fit+pos_offset, 'g', 'LineWidth', 1.5);
title('Altitude');
...
hold on
plot(time_identifikace, action_fit, 'g', 'LineWidth', 1.5);
title('Input');
legend('Input');
xlabel('Time [s]');
ylabel('Input [-]');
% axis([0 time_plot(end) -250 250]);  
